...
export default function BookingPage() {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      // Create hold right before redirecting to Stripe
      ...
      if (!currentHoldToken) {
        ...
        if (!holdRes.ok) {
          throw new Error(holdData.error || 'These dates are no longer available. Please go back and select different dates.');
        }
        currentHoldToken = holdData.holdToken;
        holdTokenRef.current = holdData.holdToken;
      ...
      const response = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          pricePerDay: Number(effectivePricePerDay),
          startDate,
          endDate,
          babySeatsQuantity: extrasData.babySeatsQuantity,
          extraDriver: extrasData.extraDriver,
          insuranceType: extrasData.insuranceType,
          selectedExtras: extrasData.selectedExtras || {},
      ...
      if (!response.ok) {
        ...
        if (response.status === 409 && errorData.code === 'DATES_UNAVAILABLE') {
          throw new Error('These dates are no longer available. Please go back and select different dates.');
        }
        const details = errorData.details?.map((d: { field: string; message: string }) => `${d.field}: ${d.message}`).join(', ');
        throw new Error(details ? `${errorData.error} — ${details}` : errorData.error || 'Failed to create checkout session');
      }

      const { url } = await response.json();

      if (url) {
        holdTokenRef.current = null; // Don't release hold on unmount — we're going to Stripe
